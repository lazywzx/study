...
# camera_projection_type: spherical
...
feature_process_size: 5472              # Resize the image if its size is larger than specified. Set to -1 for original size
...
flann_algorithm: KDTREE      # Algorithm type (KMEANS, KDTREE)
...
matching_gps_neighbors: 8             # Number of images to match selected by GPS distance. Set to 0 to use no limit (or disable if matching_gps_distance is also 0)
...
bundle_outlier_filtering_type: AUTO    # Type of threshold for filtering outlier : either fixed value (FIXED) or based on actual distribution (AUTO)
...
retriangulation_ratio: 2          # Retriangulate when the number of points grows by this ratio
...
undistorted_image_format: tif         # Format in which to save the undistorted images
...
depthmap_resolution: 2560             # Resolution of the depth maps
...
# Other params
processes: 5                  # Number of threads to use
...
osfmConfig = yaml.safe_load(default_config_yaml)
